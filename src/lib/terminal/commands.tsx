...
export async function getAutocompleteCandidates(
  ...
  if (parts.length === 1) {
    ...
    return Object.keys(COMMAND_REGISTRY).filter((cmd) =>
  ...
  if (cwd === "~") {
    ...
    const data = apiCache["/api/leaderboard"] as
...
export const COMMAND_REGISTRY: Record<string, CommandHandler> = {
  help: (args, { appendOutput }) => {
    appendOutput(
      <div style={{ lineHeight: "1.6" }}>
        ...
        >
          <li>
            <span style={{ color: "var(--fg)" }}>help</span> - Show this message
          </li>
          <li>
            <span style={{ color: "var(--fg)" }}>clear</span> - Clear terminal
            screen
          </li>
          <li>
            <span style={{ color: "var(--fg)" }}>challenges</span> - View
            available CTF challenges
          </li>
          <li>
            <span style={{ color: "var(--fg)" }}>logout</span> - End current
            session
          </li>
          <li>
            <span style={{ color: "var(--fg)" }}>whoami</span> - Display current
            user info
          </li>
          <li>
          ...
          <li>
            <span style={{ color: "var(--fg)" }}>ls</span> - List files and
            directories
          </li>
          <li>
            <span style={{ color: "var(--fg)" }}>cd &lt;dir&gt;</span> - Change
            directory
          </li>
          <li>
            <span style={{ color: "var(--fg)" }}>cat &lt;file&gt;</span> - View
            file contents
          </li>
          <li>
  ...
  ls: async (args, { state, appendOutput }) => {
        ...
        appendOutput(
          <div style={{ display: "flex", gap: "15px" }}>
            {data.categories.map((c) => (
              <span key={c} style={{ color: "var(--neon-blue)" }}>
                {c}/
              </span>
            ))}
          </div>,
    ...
    appendOutput(
      `ls: ${args[0] || targetPath}: No such file or directory`,
      "error",
    );
  },
  ...
  cat: async (args, { state, appendOutput }) => {
    ...
    if (
      ...
      targetPath === "~/teams" ||
      (targetPath.startsWith("~/challenges/") &&
    ...
    if (targetPath.startsWith("~/teams/") && targetPath.endsWith(".txt")) {
      ...
      try {
        ...
        const team = teams.find(
          (t) => t.name.replace(/\s+/g, "_") === teamName,
        );
        if (!team) {
          appendOutput(`cat: ${target}: No such file or directory`, "error");
          return;
